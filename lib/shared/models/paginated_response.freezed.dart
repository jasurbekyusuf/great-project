...
abstract class $PaginatedResponseCopyWith<T, $Res> {
  ...
  @useResult
  $Res call({int count, List<T> result});
}
...
class _$PaginatedResponseImpl<T> implements _PaginatedResponse<T> {
  const _$PaginatedResponseImpl(
      {this.count = 0, final List<T> result = const []})
      : _result = result;
  ...
  @override
  String toString() {
    return 'PaginatedResponse<$T>(count: $count, result: $result)';
  }
...
abstract class _PaginatedResponse<T> implements PaginatedResponse<T> {
  const factory _PaginatedResponse({final int count, final List<T> result}) =
